...
from model_fusion import RawGAT_ST
...
def evaluate_accuracy(data_loader, model, device):
    ...
    if args.loss == "tocsoftmax":
        criterion = TOCSoftmax().to(device)
    if args.loss == "DC_ocsoftmax":
        criterion = DC_OCSoftmax(parser1['model']).to(device)
    ...
    for batch_x, batch_y, batch_meta in data_loader:
        ...
        batch_out,x1 = model(batch_x,Freq_aug=False)
        
        if args.loss == "DC_ocsoftmax":
            batch_loss = criterion(batch_out, batch_y,x1)
        else:
...
def produce_evaluation_file(dataset, model, device, save_path):
    ...
    for batch_x, batch_y, batch_meta in data_loader:
        ...
        batch_y = batch_y.view(-1).type(torch.int64).to(device)
        batch_out,x1 = model(batch_x,Freq_aug=False)
        #if batch_out.size(-1)>1:
        #    batch_score = (batch_out[:, 1]  
        #                  ).data.cpu().numpy().ravel()     
        #else:
        batch_score1 = (batch_out[:, 0]  
                       ).data.cpu().numpy().ravel()
        batch_score = -batch_score1
...
def train_epoch(data_loader, model, lr,optimizer, device):
    ...
    for batch_x, batch_y, batch_meta in data_loader:
        ...
        batch_out,x1 = model(batch_x,Freq_aug=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--protocols_path', type=str, default='/public/home/acal2okrm7997/wmnt/LA/ASVspoof_LA_cm_protocols/ASVspoof2019_LA_cm_protocols', help='Change with path to user\'s LA database protocols directory address')
    '''
    % protocols_path/
    ...
    parser.add_argument('--loss', type=str, default="ce",
                        choices=['ce','ocsoftmax', 'tocsoftmax', 'DC_ocsoftmax'], help="loss for one-class learning")
    # backend options
    ...
    dir_yaml = os.path.splitext('/public/home/acal2okrm7997/g813_u1/zhaobo/RawGAT-ST/model_config_RawGAT_ST.yaml')[0] + '.yaml'
